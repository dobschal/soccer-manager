...
- **TA-AC-51**: Die `SpyReportCard` ist **standardmaessig zugeklappt** und in der Warning-Farbe (`bg-warning-subtle`) hervorgehoben, damit die sperrige Aufstellungs-Grafik den Taktik-Bereich nicht auseinanderzieht. Der Auf-/Zuklapp-Zustand wird in `localStorage` (`spyReport.collapsed`) gemerkt; nur ein explizites Aufklappen des Nutzers ueberschreibt die Voreinstellung.
