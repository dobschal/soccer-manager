...
},
{
  ...
  name: 'Wiki: the spy keeps reporting until the next match day',
  async run () {
    const KEYS_TO_REFRESH = ['action-cards']
    for (const topic of WIKI_SEED) {
      if (!KEYS_TO_REFRESH.includes(topic.key)) continue
  ...
  name: 'Wiki: a stand under construction is closed, not empty',
  async run () {
    const KEYS_TO_REFRESH = ['stadium']
    for (const topic of WIKI_SEED) {
      if (!KEYS_TO_REFRESH.includes(topic.key)) continue
  ...
  name: 'Wiki: rest is the default youth mode and unlimited',
  async run () {
    const topic = WIKI_SEED.find(t => t.key === 'youth-players')
    if (!topic) return
    for (const locale of ['en', 'de']) {
