...
export async function collectStatistics () {
  ...
  const [[{ daily_active_users: dailyActiveUsers }]] = await Promise.all([
    query('SELECT COUNT(*) AS daily_active_users FROM user WHERE last_login > NOW() - INTERVAL 1 DAY')
  ])
...
export async function getStatistics ({ limit = 30, offset = 0 } = {}) {
  ...
  const rows = await query(
    `SELECT id, daily_active_users, in_game_money, player_count,
            avg_player_level, avg_player_age, action_card_count, created_at
     FROM statistics
