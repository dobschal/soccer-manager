...
describe('referralHelper', () => {
  ...
  describe('claimReferralForNewUser', () => {
    it('is a no-op when email or new user id is missing', async () => {
      expect(await claimReferralForNewUser({ email: '', newUserId: 1 })).toEqual({ awarded: false })
      expect(await claimReferralForNewUser({ email: 'a@b.com', newUserId: 0 })).toEqual({ awarded: false })
    ...
    it('marks the invitation used even when the inviter has no team', async () => {
      query
        .mockResolvedValueOnce([{ id: 11, inviter_user_id: 7 }]) // invitation lookup
        .mockResolvedValueOnce([]) // inviter team lookup — no team
        .mockResolvedValueOnce([{ setting_value: 'BONUS_100K' }]) // getReferralBenefit
      ...
      // No action_card insert when there is no team
    ...
    it('awards the configured action card to the inviter team and marks the invitation used', async () => {
      query
        .mockResolvedValueOnce([{ id: 11, inviter_user_id: 7 }]) // invitation lookup
